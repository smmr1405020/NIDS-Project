...
def add_cluster_label():
    data_loader = DataLoader(test_dataset, batch_size=test_dataset.__len__(), shuffle=True)
    clustering =  OPTICS(min_samples=20)
    refined_cluster_mapping = dict()
    ...
    for batch_idx, (x, y, w) in enumerate(data_loader):
        ...
        for k, v in cluster_to_label_dict.items():
            if k == -1:
                continue
            label = np.max(v)
            label_to_cluster_dict.setdefault(label, []).append(k)

        counter = 1
        for k, v in label_to_cluster_dict.items():
            for lbl in v:
                refined_cluster_mapping[lbl] = counter
                counter += 1

        for i in range(len(cluster_assignment)):
            if cluster_assignment[i] not in refined_cluster_mapping.keys():
                refined_cluster_mapping[cluster_assignment[i]] = 0

    ref_cluster_max = np.max(list(refined_cluster_mapping.values()))
    if ref_cluster_max == 0:
        ref_cluster_max = 1

    print("Cluster assignment on Total dataset started.")
    total_dataset_loader = DataLoader(total_dataset, batch_size=total_dataset.__len__(), shuffle=False)
    for batch_idx, (x, y, w) in enumerate(total_dataset_loader):
        num_data = x.cpu().detach().numpy()
        cluster_assignment = clustering.fit_predict(num_data)
        for i in range(len(cluster_assignment)):
            cluster_assignment[i] = -1 + 2 * (refined_cluster_mapping[cluster_assignment[i]] / ref_cluster_max)

        total_dataset.add_column(cluster_assignment)
        break

    if labeled_dataset.__len__() > 0:
        print("Cluster assignment on labeled dataset started.")
        labeled_dataset_loader = DataLoader(labeled_dataset, batch_size=labeled_dataset.__len__(), shuffle=False)
        for batch_idx, (x, y, w) in enumerate(labeled_dataset_loader):
            num_data = x.cpu().detach().numpy()
            cluster_assignment = clustering.fit_predict(num_data)
            for i in range(len(cluster_assignment)):
                cluster_assignment[i] = -1 + 2 * (refined_cluster_mapping[cluster_assignment[i]] / ref_cluster_max)

            labeled_dataset.add_column(cluster_assignment)
            break

    if unlabeled_dataset.__len__() > 0:
        print("Cluster assignment on unlabeled dataset started.")
        unlabeled_dataset_loader = DataLoader(unlabeled_dataset, batch_size=unlabeled_dataset.__len__(), shuffle=False)
        for batch_idx, (x, y, w) in enumerate(unlabeled_dataset_loader):
            num_data = x.cpu().detach().numpy()
            cluster_assignment = clustering.fit_predict(num_data)
            for i in range(len(cluster_assignment)):
                cluster_assignment[i] = -1 + 2 * (refined_cluster_mapping[cluster_assignment[i]] / ref_cluster_max)

            unlabeled_dataset.add_column(cluster_assignment)
            break

    if normal_dataset.__len__() > 0:
        print("Cluster assignment on normal dataset started.")
        normal_dataset_loader = DataLoader(normal_dataset, batch_size=normal_dataset.__len__(), shuffle=False)
        for batch_idx, (x, y, w) in enumerate(normal_dataset_loader):
            num_data = x.cpu().detach().numpy()
            cluster_assignment = clustering.fit_predict(num_data)
            for i in range(len(cluster_assignment)):
                cluster_assignment[i] = -1 + 2 * (refined_cluster_mapping[cluster_assignment[i]] / ref_cluster_max)

            normal_dataset.add_column(cluster_assignment)
            break

    print("Cluster assignment on test dataset started.")
    test_dataset_loader = DataLoader(test_dataset, batch_size=test_dataset.__len__(), shuffle=False)
    for batch_idx, (x, y, w) in enumerate(test_dataset_loader):
        num_data = x.cpu().detach().numpy()
        cluster_assignment = clustering.fit_predict(num_data)
        for i in range(len(cluster_assignment)):
            cluster_assignment[i] = -1 + 2 * (refined_cluster_mapping[cluster_assignment[i]] / ref_cluster_max)

        test_dataset.add_column(cluster_assignment)
        break
...
def train_full_model(load_pretrained_ae=False, load_trained_ae=False, load_rec_model=False, not_caring=False):
    ...
    if not load_rec_model:
        pretrain_ae(rec_model, normal_dataset, args.reconstruction_based_ae_pretrain_path_normaldata,
                    pretrain_ae_normaldata_epochs)
